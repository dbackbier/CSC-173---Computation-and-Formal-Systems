...
uint64_t hash_IntHashSet(IntHashSet set) {
	uint64_t h = 1469598103934665603ULL;
	for (int i = 0; i < IntHashSet_count(set); i++) {
		for (Node *p = set->buckets[i]; p; p = p->next) {
			h ^= (uint64_t)p->element;
			h *= 1099511628211ULL;
		}
	}
	return h;
 ...
 */
int IntHashSet_get(IntHashSet this, int index) {
	if (IntHashSet_isEmpty(this)) {
		return -1;
	}
	return this->buckets[index]->element;
}
