...
typedef struct DFAStateEntry DFAState;
typedef struct DFAStateTable DFAStateTable;
...
extern int DFAStateTable_hash(DFAStateTable *table, IntHashSet set);
 ...
 */
extern IntHashSet NFA_get_transitions(NFA nfa, int state, unsigned char sym);

/**
 * For the given NFA, add the state dst to the set of next states from
 * state src on input symbol sym.
 */
extern void NFA_add_transition(NFA nfa, int src, unsigned char sym, int dst);
